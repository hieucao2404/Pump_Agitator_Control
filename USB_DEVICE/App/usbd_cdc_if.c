...
static int8_t CDC_Control_FS(uint8_t cmd, uint8_t *pbuf, uint16_t length) {
  ...
  case CDC_SET_CONTROL_LINE_STATE: {
    /* pbuf contains the control line state: bit0 = DTR, bit1 = RTS
       When the host opens the virtual COM port it typically asserts DTR.
       Use this to notify the comm handler about connection state. */
    uint16_t ctrl_state = 0;
    if (pbuf != NULL && length >= 2) {
      ctrl_state = (uint16_t)pbuf[0] | ((uint16_t)pbuf[1] << 8);
    }

    if (ctrl_state & 0x01) {
      /* DTR asserted -> host opened port */
      comm_set_usb_connected(1);
    } else {
      /* DTR cleared -> host closed port */
      comm_set_usb_connected(0);
    }
  }
...
static int8_t CDC_Receive_FS(uint8_t *Buf, uint32_t *Len) {
  /* USER CODE BEGIN 6 */
  // Pass received data to communication handler
  comm_receive_data(Buf, *Len);
